...
np.savetxt("test_data_{}.txt".format(date_append),np.transpose([t,samp]),fmt=['%.5f','%.5f'],header="Freq={}\ttau={}\tamp={}".format(seed_freq,seed_tau,seed_amp))
...
freq_bins = np.linspace(np.min(freq_list),np.max(freq_list),100)
...
fig = cr.corner(samples, labels=["$F$", r"$\tau$", "$A$"],
                      truths=[seed_freq, seed_tau, seed_amp],
                      bins = [freq_bins,tau_bins,amp_bins])
fig.savefig("triangle_low_amp_{}.png".format(date_append))
